...
DesDocu as Documento, NumDoc as Numero from TabEnfm  inner join TabDocu on TabEnfm .IdDocu=TabDocu.IdDocu 
go
--Mostrar solo un registro específico
...
DesDocu as Documento, NumDoc as Numero from TabDoct inner join TabDocu on Tabdoct.IdDocu=TabDocu.IdDocu 
go
--Mostrar solo un registro específico
...
@TelApo int
as
insert into TabPac(ApePat,ApeMat,NomPac,SexPac,FecNac,IdDist,IdProv,IdDpto,IdDistNac,IdProvNac,IdDptoNac,DomPac,IdRelg,IdEciv,IdOcup,Centra,TieDes,TieRes,IdGins,IdDocu,Numdoc,IdIdma,RefPor,AppApo,ApmApo,NomApo,DirApo,TelApo) 
values(@ApePat,@ApeMat,@NomPac,@SexPac,@FecNac,@IdDist,@IdProv,@IdDpto,@IdDistN,@IdProvN,@IdDptoN,@DomPac,@IdRelg,@IdEciv,@IdOcup,@Centra,@TieDes,@TieRes,@IdGins,@IdDocu,@Numdoc,@IdIdma,@RefPor,@AppApo,@ApmApo,@NomApo,@DirApo,@TelApo)
go
--Mostrar
...
IdDistNac as Distrito_Nac, 
IdProvNac as Provincia_Nac, 
IdDptoNac as Departamento_Nac,
DomPac as Domicilio,
IdRelg as Religion, 
IdEciv as Est_Civil,
IdOcup as Ocupación,
...
Create procedure sp_LlenarCamposPart
...
------------------------------------------------------
--Diseño de Procedimientos para los Reportes
------------------------------------------------------
--Reporte HraCli
Create procedure R_HraCli
@nrohra int
as
select nrohra, nroing, fechra, Apepat, Apemat, nompac, fecnac, desdist, desprov, desdpto, dompac, desrelg, deseciv,
desocup, centra, tiedes, tieres, desgins, desdocu, numdoc, desidma, refpor, appapo, apmapo, nomapo, dirapo, telapo,
sintma, desdiag, fecing, fecegr, tiehos, destiph, descond
from 
hracli,tabpac,tabdist,tabprov,tabdpto,tabrelg,TabEciv,tabocup,tabgins,tabdocu,tabidma,tabdiag,tiphosp,tabcond
where
hracli.idpcte=tabpac.idpcte
and tabpac.iddpto=tabdist.iddpto
and tabpac.idprov=tabdist.idprov 
and tabpac.iddist=tabdist.iddist
and tabpac.idprov=tabprov.idprov 
...
and tabpac.idgins=tabgins.idgins
and tabpac.Iddocu=tabdocu.Iddocu
and hracli.Idtiph=tiphosp.idtiph
and hracli.Idcond=tabcond.idcond
and tabpac.ididma=tabidma.ididma
and hracli.iddiag=tabdiag.iddiag
and hracli.nrohra=@nrohra
go

--Reporte Parte Diario 	   
Create proc R_PartDia
@IdPart int
as
select 
tabdoct.Apepat + ' ' + tabdoct.Apemat as Doctor,
cabpartdia.Fecpar as fecha,
detpartdia.NroHra as HraCli,
TabPac.Apepat + ' ' + TabPac.ApeMat + ' ' + TabPac.NomPac as Paciente, 
Tabpac.SexPac as Sexo,
edad=(Select case when month(getdate()) - month(TabPac.FecNac) < 0 or day(getdate()) - day(TabPac.FecNac) < 0 
then year(getdate()) - year(TabPac.FecNac) - 1 
...
then year(getdate()) - year(TabPac.FecNac)end),
ContPac.DescPac as Estado,
detpartdia.Iddiag as Cat_Diagnostico,
Tabdiag.DesDiag as Des_Diagnostico,
TipPac.CodTipPac as Categoría,
tabdist.desdist as Procedencia,
(select count(SexPac) as Sexo from tabpac,hracli,detpartdia where SexPac='F' 
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and detpartdia.idpart=@IdPart )as Mujeres,
(select count(SexPac) as Sexo from tabpac,hracli,detpartdia where SexPac='M' 
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and detpartdia.idpart=@IdPart ) as Varones,
(select count(Tabpac.Idpcte) as Niños from tabpac,hracli,detpartdia where 
(Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
then year(getdate()) - year(tabpac.Fecnac)end)<=10
and tabpac.idpcte=hracli.idpcte and hracli.nrohra=detpartdia.nrohra and detpartdia.idpart=@IdPart) as Niños,
(select count(Tabpac.Idpcte) as Jovenes from tabpac,hracli,detpartdia where 
(Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
then year(getdate()) - year(tabpac.Fecnac)end)between 11 and 17
and tabpac.idpcte=hracli.idpcte and hracli.nrohra=detpartdia.nrohra and detpartdia.idpart=@IdPart) as Jovenes,
(select count(Tabpac.Idpcte) as Adultos from tabpac,hracli,detpartdia where 
(Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
then year(getdate()) - year(tabpac.Fecnac)end)between 18 and 60
and tabpac.idpcte=hracli.idpcte and hracli.nrohra=detpartdia.nrohra and detpartdia.idpart=@IdPart) as Adultos,
(select count(Tabpac.Idpcte) as Ancianos from tabpac,hracli,detpartdia where 
(Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
then year(getdate()) - year(tabpac.Fecnac)end)>=60
and tabpac.idpcte=hracli.idpcte and hracli.nrohra=detpartdia.nrohra and detpartdia.idpart=@IdPart) as Ancianos,
(select count(idcpac) from detpartdia where idcpac=2 and idpart=@IdPart) as Nuevos,
(select count(idcpac) from detpartdia where idcpac=3 and idpart=@IdPart) as Continuadores,
(select count(idcpac) from detpartdia where idcpac=4 and idpart=@IdPart) as Reingresantes
from Tabpac, HraCli,detpartdia,ContPac,tabdist,tabprov,tabdpto,TipPac,TabDiag,tabdoct,cabpartdia 
where 
ContPac.IdcPac=detpartdia.IdcPac 
and TipPac.IdTipPac = detpartdia.IdTipPac
and tabpac.iddpto=tabdist.iddpto
...
and HraCli.NroHra=detpartdia.NroHra 
and TabPac.IdPcte =HraCli.IdPcte 
and detpartdia.Iddiag =tabdiag.iddiag
and tabdoct.iddoct=cabpartdia.iddoct
and cabpartdia.idpart=detpartdia.idpart
and detpartdia.IdPart=@IdPart
go

--Reporte Quincenas
create proc R_EstQui
@Fecha1 Datetime,
@Fecha2 Datetime
as
select count(*) as Varones_Indigentes,
(select count(*) from detpartdia,cabpartdia,tippac,tabpac,hracli 
where tippac.codtippac <> 'E'
and tippac.idtippac=detpartdia.idtippac 
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and tabpac.sexpac='M'
and detpartdia.idpart=cabpartdia.idpart
and cabpartdia.fecpar between @Fecha1 and @Fecha2
)as Varones_Pagantes,
(select count(*) from detpartdia,cabpartdia,tippac,tabpac,hracli  
where tippac.codtippac='E'
and tippac.idtippac=detpartdia.idtippac 
...
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and (Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
then year(getdate()) - year(tabpac.Fecnac)end)<=10
and detpartdia.idpart=cabpartdia.idpart
...
from detpartdia,cabpartdia,tippac,tabpac,hracli  
where tippac.codtippac='E'
and tippac.idtippac=detpartdia.idtippac 
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
...
and hracli.nrohra=detpartdia.nrohra
and detpartdia.idpart=cabpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar)= @Año
) as CSoc_Pagantes,
(
select count(*) from detpartdia,cabpartdia,tippac,tabpac,hracli  
where tippac.codtippac='E'
...
(
select count(*) from tabpac,hracli,cabpartdia,detpartdia where idprov=1
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and detpartdia.idpart=cabpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar)= @Año
...
(
select count(*) from tabpac,hracli,detpartdia,cabpartdia
where tabpac.sexpac='M'
and tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and detpartdia.idpart=cabpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar)= @Año
...
(
select count(*) from tabpac,hracli,detpartdia,cabpartdia
where tabpac.idpcte=hracli.idpcte
and hracli.nrohra=detpartdia.nrohra
and (Select case when month(getdate()) - month(tabpac.Fecnac) < 0 or day(getdate()) - day(tabpac.Fecnac) < 0 then
year(getdate()) - year(tabpac.Fecnac) - 1 
when month(getdate()) - month(tabpac.Fecnac) >= 0 or day(getdate()) - day(tabpac.Fecnac) >= 0 
...
from detpartdia,cabpartdia 
where cabpartdia.idpart=detpartdia.idpart 
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar)= @Año
go


create proc sp_Diagnosticos
@Mes int,
@Año int
as
select iddiag as Diagnosticos,count(iddiag) as Casos 
from detpartdia d ,cabpartdia c 
where month(c.fecpar)=@Mes 
and year(c.fecpar)= @Año
and d.idpart= c.idpart 
group by iddiag
go
...
--Reporte estadistico doctor
create proc R_EstMed
@Mes_txt varchar(15),
@Mes int,
@Año int,
@fecha_actual datetime 
as
select Apepat as doctor,
(select count(*) from detpartdia,cabpartdia 
where detpartdia.idcpac=2 
and tabdoct.iddoct=cabpartdia.iddoct
and cabpartdia.idpart=detpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar) = @Año
)as Nuevos,
(select count(*) from detpartdia,cabpartdia 
where detpartdia.idcpac=3
and tabdoct.iddoct=cabpartdia.iddoct
and cabpartdia.idpart=detpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar) = @Año
)as Continuadores,
(select count(*) from detpartdia,cabpartdia 
where detpartdia.idcpac=4
and tabdoct.iddoct=cabpartdia.iddoct
and cabpartdia.idpart=detpartdia.idpart
and month(cabpartdia.fecpar)= @Mes and year(cabpartdia.fecpar) = @Año
)as Reingresantes,
